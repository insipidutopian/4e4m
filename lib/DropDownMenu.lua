...
function DropDownMenu.new( params )
    ...
    function dropDownMenu.onRowRender( event )
        ...
        local rowTitle  = display.newText(row, params.value, rowWidth/2, 0, rowWidth, rowHeight, "fonts/kellunc.ttf", fontSize)
        rowTitle:setFillColor(0.9,0,0)
    ...
    button:setFillColor( 0.1, 0, 0 )
    
    -- Instantiate Button label
    buttonLabel = display.newText(dropDownMenu, cDefaultButtonValue, 0, 0, 0, buttonHeight, "fonts/kellunc.ttf", fontSize)
    buttonLabel.y = (buttonHeight * 0.5) - (fontSize * 0.5)
    buttonLabel.x = cDefaultCellXPadding
    --buttonLabel.y = buttonWidth * 0.5
    
    buttonLabel:setFillColor(0.8,0,0)
    ...
    function dropDownMenu:touch(event)
        ...
        if event.phase == "began" then
            button:setFillColor(0.8,0.2,0,2, 0.4)
        elseif event.phase == "moved" then
        
        elseif event.phase == "ended" then
            self:hideTable(not isTableHidden)
            if isTableHidden then
                button:setFillColor(0.8,0.2,0.2)
            else
                button:setFillColor(0.8,0.2,1, 0.9)
            end
        end
